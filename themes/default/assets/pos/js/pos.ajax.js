...
$(document).ready(function () {
	...
	$(document).on('change', '.rserial', function () {
		...
		var all_serial 	= $(this).closest('tr').attr('sp');
		...
		var exists 		= '';
		if(serial_no != ""){
			$.each(all_serial.split(','), function(i, val){
				is_serial = val.split('_');
				arr_serial.push(is_serial[0]);
				if(serial_no == is_serial[0]){
					if(is_serial[1] == 0){
						bootbox.alert('This serial is sold out!');
						have_serial = '0';
					} else {
						have_serial = '1';
					}
			...
			$('.rserial').each(function(){
				if($(this).val() != ""){
					if(exists == $(this).val()){
						bootbox.alert('This serial is already use!');
						have_serial = '0';
					} 
					exists = $(this).val();
				}
			...
			if($.inArray(serial_no, arr_serial) == -1) {
				bootbox.alert('This serial is invalid!');
				have_serial = '0';
			}
		}
		
		positems[item_id].row.serial = $(this).val();
		positems[item_id].row.ser 	 = have_serial;
	...
	$('#sale_status').change(function (e) {
		localStorage.setItem('sale_status', $(this).val());
	});
	if (sale_status = localStorage.getItem('sale_status')) {
		$('#sale_status').select2('val', sale_status);
		$('#sale_status_1').val(sale_status);
	} else {
		$('#sale_status_1').val('completed');
	}
	...
	$(document).on('click', '.edit', function () {
		...
		serial_input 	= row.children().children('.rserial').val();
		var net_price 	= unit_price;
		$('#prModalLabel1').text(item.row.name + ' (' + item.row.code + ')');
		...
		$('#all_serial').val(all_serial);
		$('#row_id').val(row_id);
		$('#item_id').val(item_id);
		$('#pserial').val(serial_input);
		
		var remove	= '<button type="button" data="" class="removefile btn btn-danger">&times;</button>';
		
		var input 	= "";
		if(is_serial!="" && is_serial!="undefined"){
	...
	$(document).on('click', '#editItem', function () {
		...
		positems[item_id].row.serial = pserial;
		localStorage.setItem('positems', JSON.stringify(positems));
		$('#prModal').modal('hide');
	 ...
	 * Seraial Change 
	 ----------------------- */
	$(document).on('change', '.pserial', function() {
		var all_serial 	= $('#all_serial').val();
		var serial_no	= $(this).val();
		...
		if(serial_no != ""){
			...
			$('.rserial').each(function(){
				if($(this).val() == serial_no){
					bootbox.alert('This serial is already use with other item!');
					$('#editItem').attr("disabled", true);
				}
			});
...
function loadItems() {
	...
	if (localStorage.getItem('positems')) {
		...
		var pro_id 			= '';
		var count_serial 	= 0;
		var sale_status		= localStorage.getItem('sale_status')?localStorage.getItem('sale_status'):'completed';
		...
		$.each(positems, function (i, e) {
			...
			var product_id 		= item.row.id, 
				...
				sep 			= item.row.sep, 
				is_serial 		= item.row.have_serial;
			...
			unit_price = formatDecimal((unit_price-item_discount) + 0.000001);
				...
				if (pr_tax !== false) {
					if (pr_tax.type == 1) {
						...
						if (item_tax_method == '0') {
							...
							pr_tax_val = formatDecimal(((item.row.real_unit_price) * parseFloat(pr_tax.rate)) / 100);
							pr_tax_rate = formatDecimal(pr_tax.rate) + '%';
						}
			...
			var i = 1;
			for(a in combo_arr){
				combo_ += '<div style="border-bottom: 1px solid rgb(204, 204, 204);">#'+i+' '+ combo_arr[a]['name'] + ' ('+combo_arr[a]['code']+')</div>';
				i++;
			}
			...
			if(layouts != 3) {
				if(pos_settings.show_product_code == 0) {
					...
					if(pos_settings.show_item_img != 0) {
						tr_html += '<span class="showimg" style="width:500px;"><table class="table table-bordered"><tr><th>Image</th>'+(item_type == 'combo' ? '<th>Description</th>' : '')+(item_type == 'combo' ? '<th style="width:115px;">Combo Items</th>' : '')+'</tr><tr><td><a href="'+site.base_url+'assets/uploads/'+picture+'" data-toggle="lightbox"><img src="'+site.base_url+'assets/uploads/'+picture+'" alt="' +item_name+ '" style="width:80px;" class="img-thumbnail" /></a></td>'+(item_type == 'combo' ? '<td>'+item_details+'</td>' : '')+(item_type == 'combo' ? '<td style="width:250px;"><table>'+combo_+'</table></td>' : '')+'</tr></table></span>';
					}
				}else{
			...
			//======= Check Serial is Correct or not =========//
			if(item.row.ser){
				serial.push(item.row.ser);
			} 
			//===================== End ======================//
			...
			if(is_serial == 1){
				...
				qtychk 		+= item_qty;
				if(item_serial != 0){
					var itemserial = item_serial.split(',');
					$.each(itemserial, function( index, value ) {
						count_serial++;
					});
				}
			}
			...
			tr_html += '<td><input class="form-control kb-pad text-center rquantity" style="width:50px;" name="quantity[]" type="text" value="' + formatDecimal(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();" readonly></td>';
            ...
            if (item_promotion == 1){
				...
				tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney((parseFloat(item_price)  * parseFloat(item_qty))) + '</span><input type="hidden" name="grand_total[]" value="' + formatMoney((parseFloat(item_price) * parseFloat(item_qty))) + '"></td>';
			}
			...
			if (item_promotion == 1){
				...
				ftotal += formatDecimal(item_price * item_qty);
		...
		if (site.settings.purchase_serial == 1 && sale_status == 'completed') {
			if(check_serial){
				$('#payment').attr('disabled', 'disabled');
				//console.log(qtychk +'=='+ count_serial);
				//console.log(serial);
				if(qtychk == count_serial){
					if($.inArray('0', serial) == -1) {
			...
			$('.rserial').attr('disabled', 'disabled');
		}
		...
		if (posdiscount = localStorage.getItem('posdiscount')) {
			...
			if (ds.indexOf("%") !== -1) {
				var pds = ds.split("%");
				if (!isNaN(pds[0])) {
					order_discount = formatDecimal(((ftotal) * parseFloat(pds[0])) / 100 + 0.000001);
					dis_per = '(' + ds + ') ' + formatDecimal(((ftotal) * parseFloat(pds[0])) / 100 + 0.000001);
				} else {
		...
		if(java_applet == 1) {
			...
			bill_totals += '<tr><td colspan="4" style="text-align:right;font-weight:bold;">'+lang_total_payable+'</td><td style="text-align:right;font-weight:bold;font-size:18px;">'+formatMoney(gtotal)+'</td></tr>';
			bill_totals += '<tr><td colspan="4" style="text-align:right;font-weight:bold;" >Total Amount KH</td><td style="text-align:right;font-weight:bold;font-size:18px;">'+formatSA(parseFloat(bill_amount_kh).toFixed(0))+' ៛</td></tr></tfoot>';
 ...
 * @returns {Boolean}
 ...
 function add_invoice_item(item) {
 	...
 	if (count == 1) {
 		positems = {};
 		if ($('#poswarehouse').val() && $('#poscustomer').val() && $('#sale_status').val()) {
 			$('#poscustomer').select2("readonly", true);
 			$('#poswarehouse').select2("readonly", true);
			$('#sale_status').select2("readonly", true);
 		} else {

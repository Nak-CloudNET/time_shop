...
if (from_warehouse = localStorage.getItem('from_warehouse')) {
	...
	$(document).on('change','.rserial',function(){
		...
		if(serial_num){
			...
			if($.inArray(serial_num, allSerial.split(',') ) == -1) {
				bootbox.alert('This serial is invalid!');
				have_serial = '0';
			}
		}
		
		toitems[item_id].row.serial  = serial_num;
		toitems[item_id].row.ser	 = have_serial;
		localStorage.setItem('toitems', JSON.stringify(toitems));
		loadItems();
	});
 ...
 function loadItems() {
    ...
    if (localStorage.getItem('toitems')){
        ...
        $.each(toitems, function (){
            ...
            var product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_cost = item.row.cost, item_qty = item.row.qty, item_bqty = item.row.quantity_balance, item_expiry = item.row.expiry, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_serial = item.row.serial, item_name = item.row.name.replace(/'/g, "&#039;"), picture = item.row.image, description = item.row.product_details, serial_no = item.row.serial_no, sep = item.row.sep, is_serial = item.row.have_serial;
            ...
            $('#choose_serial').select2();
        });

		if(site.settings.purchase_serial == 1){
			if(check_serial){
				$('#payment').attr('disabled', 'disabled');
				//console.log(chkQty +'=='+ count_serial);
				//console.log(serial);
				if(chkQty == count_serial){
		...
		var tfoot = '<tr id="tfoot" class="tfoot active"><th class="text-center">Image</th><th class="text-center">Product Name (Product Reference)</th><th class="text-center">Serial Number</th><th class="text-center">' + formatNumber(parseFloat(count) - 1) + '</th>';

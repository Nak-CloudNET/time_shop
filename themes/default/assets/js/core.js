...
function SerialNumber(x) {
	var serial = '';
	if(x == null || x == ''){
		serial += '<div class="text-center"><span class="label label-primary">'+''+'</span></div>';
	}else{
		var serial_number = x.split(",");
		var i;
		var serial = '';
		for (i = 0; i < serial_number.length; i++) {
			serial += '<div class="text-center"><span class="label label-primary">'+serial_number[i]+'</span></div>';
		}
	}
    return serial;
}
...
function pqFormatPurchaseReportsII(x) {
	var d = '';  
    if (x != null){
        var  pqc = x.split("___");
		if(pqc.length > 0){
...
function row_status(x) {
    if(x == null) {
        ...
        return '<div class="text-center"><span class="label label-success">'+lang[x]+'</span></div>';
    } else if(x == 'partial' || x == 'partial_payment' ||x == 'transferring' || x == 'busy'  || x == 'processing'  || x == 'ordered') {
        return '<div class="text-center"><span class="label label-info">'+lang[x]+'</span></div>';
...
function partial(x){
    if(x == 'partial_payment'){
		//var x = 'partial_payment';
        return '<div class="text-center"><span class="label label-info">'+lang[x]+'</span></div>';
    } 
	if(x == 'pending'){
		 return '<div class="text-center"><span class="label label-warning">'+lang[x]+'</span></div>';
...
$(document).ready(function() {
	...
	$('body').on('click', '.document_link td:not(:first-child, :nth-child(2), :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'documents/modal_view/' + $(this).parent('.document_link').attr('id')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'products/view/' + $(this).parent('.product_link').attr('id');
    });
	$('body').on('click', '.service_link td:not(:first-child, :nth-child(2), :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'services/modal_view/' + $(this).parent('.service_link').attr('id')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'products/view/' + $(this).parent('.product_link').attr('id');
    });
	// Purchases
	...
	$('body').on('click', '.return_purchase_link td:not(:first-child :last-child)', function() {
    ...
    $('body').on('click', '.purchase_order_link td:not(:first-child :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'purchases_order/modal_view/' + $(this).parent('.purchase_order_link').attr('id')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'purchases/view/' + $(this).parent('.purchase_link').attr('id');
    });
	...
	$('body').on('click', '.notification_link td:not(:first-child :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'notifications/modal_view/' + $(this).parent('.notification_link').attr('id')});
        $('#myModal').modal('show');

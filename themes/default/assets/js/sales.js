$(document).ready(function (e) {
	...
	$(document).on('click', '.edit', function () {
		...
		serial_no    	 = row.children().children('.serial_no').val(),
		...
		$('#row_id').val(row_id);
		$('#item_id').val(item_id);
		$('#pserial').val(serial_no);
		$('#all_serial').val(group_serial);
	...
	$(document).on('click', '#editItem', function () {
		...
		$('.pserial').each(function(){
			var pserial = $('#pserial').val();
			all_serial.push(pserial);
		});
		...
		*/
		...
		slitems[item_id].row.serial = all_serial; //$('.pserial').val();
		localStorage.setItem('slitems', JSON.stringify(slitems));
		$('#prModal').modal('hide');
	...
	$(document).on('change', '.serial_no', function () {
		...
		slitems[item_id].row.serial = $(this).val();
		slitems[item_id].row.ser 	 = have_serial;
	 	localStorage.setItem('slitems', JSON.stringify(slitems));
	 	loadItems();
...
//localStorage.clear();
function loadItems(status_sale) {
	...
	if (localStorage.getItem('slitems')) {
		...
		$.each(slitems, function () {
			...
			sale_status			= status_sale?status_sale:$('#slsale_status').val();
			...
			if (site.settings.product_serial == 1) {
				tr_html += '<td><input class="form-control text-center serial_no" '+(is_serial == 1 ? '' : 'readonly')+' name="serial[]" type="text" value="' + item_serial + '" data-id="' + row_no + '" data-item="' + item_id + '" id="serial_' + row_no + '" "></td>';
			}
			...
			if (item_promotion == 1){
				total += formatDecimal(((parseFloat(item_pro_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty)));
			...
			if (sltax2 = localStorage.getItem('sltax2')) {
				$.each(tax_rates, function () {
					if (this.id == sltax2) {
						...
						if (this.type == 1) {
							invoice_tax = formatDecimal(((ftotal - order_discount) * this.rate) / 100);
						}
					}
				});
			}
		...
		var gtotal = parseFloat(((ftotal + invoice_tax) - total_discount) + shipping);
		$('#total').text(formatMoney(ftotal));
		...
		if (count > 1) {
			$('#slcustomer').select2("readonly", true);
			$('#slwarehouse').select2("readonly", true);
			var sale_status = localStorage.getItem('sales_status');
			if (sale_status == 'completed') {
				$('#slsale_status').select2("readonly", true);
			} else if (sale_status == 'pending') {
				$('#slsale_status option[value="ordered"]').remove();
			}
		}
		//audio_success.play();
	}

                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data === false) {
							...
							amount 			= $("#amount_1").val();
							var last_amount = (data.deposit_amount == null ? 0 : parseFloat(data.deposit_amount)) + parseFloat(amount);

                            $('#dp_details_1').html('<small>Customer Name: ' + data.name + '<br>Amount: <span class="deposit_total_amount">' + (data.deposit_amount == null ? 0 : data.deposit_amount) + '</span> - Balance: <span class="deposit_total_balance">' + last_amount + '</span></small>');
        ...
        if (localStorage.getItem('reitems')) {
            ...
            $.each(reitems, function () {
                ...
                if (site.settings.product_serial == 1) {
                    tr_html += '<td class="text-right"><input class="form-control input-sm rserial" name="serial[]" readonly type="text" id="serial_' + row_no + '" value="' + item_serial + '"></td>';
                }
                ...
                tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty)) - (item_discount * item_qty) ) + '</span></td>';
				
                tr_html += '<td class="text-center"><i class="fa fa-times tip pointer redel" id="' + row_no + '" title="Remove" disabled style="cursor:pointer;pointer-events: none;"></i></td>';
                newTr.html(tr_html);
...
<div class="row">
	<div class="col-md-12">
		...
		if ($inv->payment_status == 'paid') {
			...
			echo '<div class="alert alert-warning"><p>' . lang('payment_status_not_paid') . ' ' . lang('payment_status') . ': <strong>' . $inv->payment_status . '</strong> & ' . lang('paid_amount') . ' <strong>' . $this->erp->formatMoney($inv->paid) . '</strong></p></div>';
		}
		?>
	</div>
</div>
                ...
                <div class="row">
                    <div class="col-lg-12">
						...
						<div class="col-md-4">
                            <div class="form-group">
                                <?= lang("reref_no", "reref"); ?>
                                <?php echo form_input('reref_no', '', 'class="form-control input-tip" id="reref_no"'); ?>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <?= lang("reference_no", "slref"); ?>
                                <?php echo form_input('reference_no', $inv->reference_no, 'class="form-control input-tip" id="reref" readonly'); ?>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
						...
						<div class="col-md-4">
                            <div class="form-group">
                                ...
                                <?php echo form_input('order_discount', ($inv->order_discount_id?$inv->order_discount_id:0), 'class="form-control input-tip" id="rediscount" readonly required="required"'); ?>
                            </div>
                        </div>
                            ...
                            <div id="bottom-total" class="well well-sm" style="margin-bottom: 0;">
                                <table class="table table-bordered table-condensed totals" style="margin-bottom:0;">
                                    <tr class="warning">
                                        ...
                                        <td><?= lang('order_discount') ?> <span class="totals_val pull-right" id="tdis">0.00</span></td>

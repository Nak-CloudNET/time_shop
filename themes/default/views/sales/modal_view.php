    ...
    <div class="modal-content">
        <div class="modal-body">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover table-striped print-table order-table">
                    ...
                    <tr>
						...
						<th><?= lang("unit"); ?></th>
                        <th><?= lang("Price_Before_Tax"); ?></th>
                        <?php
                        	if ($Settings->tax1) {
                           	 	echo '<th>' . lang("tax") . '</th>';
                        	}
						?>
                        <th><?= lang("price_after_tax"); ?></th>
						<th><?= lang("quantity"); ?></th>
                        <th><?= lang("subtotal"); ?></th>
                        <?php
                    ...
                    ?>
                        <tr>
                            ...
                            <td style="text-align:right; width:100px;"><?= $row->subtotal!=0?$this->erp->formatMoney($row->unit_price - ($row->item_tax/$row->quantity)):$free; ?></td>
                            <?php
                            if ($Settings->tax1) {
                                $tax=substr($row->tax_name,-3);
                                echo '<td style="width: 100px; text-align:right; vertical-align:middle;">' . ($row->item_tax != 0 && $row->tax_code ? '<small>('.$tax.')</small>' : '') . ' ' . $this->erp->formatMoney($row->item_tax/$row->quantity) . '</td>';
                            }
							?>
                            ...
                            <td style="text-align:right; width:120px;"><?= $row->subtotal!=0?$this->erp->formatMoney($row->subtotal + $row->item_discount):$free; 
								$stotal += ($row->subtotal + $row->item_discount);
                                ?>
							</td>
                            <?php
                            if ($Settings->product_discount && $inv->product_discount != 0) {
                    ...
                    <?php if ($inv->grand_total != $inv->total) { ?>
                        <tr>
							...
							<td style="text-align:right; padding-right:10px;"><?= $this->erp->formatMoney($inv->total - $inv->product_discount); ?></td>
                        </tr>
                    <?php } ?>
                    ...
                    <?php if ($inv->order_discount != 0) {
                        echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang("Sale_Discount") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->erp->formatMoney($inv->order_discount) . '</td></tr>';
                    }
                    ?>
                    <?php if ($Settings->tax2 && $inv->order_tax != 0) {
                        echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;">' . lang("Sale_Tax") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->erp->formatMoney($inv->order_tax) . '</td></tr>';
                    }
                    ?>
            ...
            <?php if (!$Supplier || !$Customer) { ?>
                <div class="buttons">
					<?php
						$attachment = $inv->attachment.','.$inv->attachment1.','.$inv->attachment2;
						if($attachment){
					...
					?>
						<div class="btn-group">
								<ul class="enlarge">
									<li>
                            <a href="<?= site_url('welcome/download/' . $attach) ?>" class="tip btn btn-primary" title="<?= lang('attachment') ?>">
                                <i class="fa fa-chain"></i>
                                <span class="hidden-sm hidden-xs"><?= lang('attachment') ?></span>
                            </a>
							<span><a data-toggle="lightbox" href="<?=base_url().'files/'.$attach;?>"><img class="img-thumbnail" style="width:200px;" alt="<?=$attach;?>" src="<?=base_url().'files/'.$attach;?>"></a></span>
									</li>
								</ul>
                        </div>
					<?php

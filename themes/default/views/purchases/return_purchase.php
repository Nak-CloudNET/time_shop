<script type="text/javascript">
    ...
    function loadItems() {
        ...
        if (localStorage.getItem('reitems')) {
            ...
            $.each(reitems, function () {
				...
				tr_html += '<td><input name="product_id[]" type="hidden" is_serial="'+is_serial+'" class="rid" value="' + product_id + '"><input name="serial_input[]" type="hidden" class="serial_input" value="' + (serial_item ? serial_item : 0) + '"><input name="serial_number[]" type="hidden" class="serial_number" value="' + (serial_no ? serial_no : 0) + '"><input name="purchase_item_id[]" type="hidden" class="rpiid" value="' + purchase_item_id + '"><input name="purchase_id[]" type="hidden" class="purchase_id" value="' + purchase_id + '"><input name="product[]" type="hidden" class="rcode" value="' + item_code + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><input name="product_option[]" type="hidden" class="roption" value="' + item_option + '"><input name="part_no[]" type="hidden" class="rpart_no" value="' + item_supplier_part_no + '"><span class="sname" id="name_' + row_no + '">' + item_name + ' (' + item_code + ')'+(sel_opt != '' ? ' ('+sel_opt+')' : '')+' <span class="label label-default">'+item_supplier_part_no+'</span></span><i class="pull-right fa fa-edit tip edit" id="' + row_no + '" data-item="' + item_id + '" title="Edit" style="cursor:pointer;"></i></td>';
                ...
                if (site.settings.product_discount == 1) {
                    tr_html += '<td class="text-right"><input class="form-control input-sm rdiscount" name="product_discount[]" type="hidden" id="discount_' + row_no + '" value="' + item_ds + '"><span class="text-right sdiscount text-danger" id="sdiscount_' + row_no + '">' + (item_ds ? '(' + item_ds + ')' : '') + ' ' + formatMoney((item_discount * item_qty)) + '</span></td>';
                }
                ...
                total += formatDecimal(((parseFloat(item_cost - item_discount) + parseFloat(pr_tax_val)) * parseFloat(item_qty)));
			...
			if(item.row.serial != null){
				...
				if(sr_item.length > 0){
					for(var i=0; i < sr_item.length; i++){
						input += '<div style="display:flex;padding:5px 0px 5px 0px" class="remove"><input type="text" class="form-control serial_no" name="serial[]" style="pointer-events: none;" value='+sr_item[i]+'>'+remove+'</div>';
					}
				}
				
				if(sr_item.length < qtyr){
					var rowcount= qtyr - sr_item.length;
					for(var j=0;j<rowcount;j++){
						input += '<div style="display:flex;padding:5px 0px 5px 0px" class="remove"><input type="text" class="form-control serial_no" style="pointer-events: none;" name="serial[]">'+remove+'</div>';
					}
				}
			}else{
				...
				if(ser_arr != ""){
					if(ser_arr.length > 0){
						for(var i=0; i< qtyr;i++){
							input += '<div style="display:flex;padding:5px 0px 5px 0px" class="remove"><input type="text" class="form-control serial_no" name="serial[]" style="pointer-events: none;" value='+((ser_arr[i] == undefined || ser_arr[i] == 0)? "" : ser_arr[i])+'>'+remove+'</div>';
						}
					}
				}else{
					for(var m=0; m < qtyr; m++){
	...
	var qty_serial = 0;
	$(document).on("focus", '#pquantity', function () {
        qty_serial = $(this).val();
    });
	$(document).on('change','#pquantity',function(){
		var row = $(this).closest('tr');
		var qty = $(this).val();
		var oqty = reitems[item_id].row.oqty;
		...
		if(qty < oqty){
			...
			alert('Quantity is bigger than quantity received');
			$('#pquantity').val(qty_serial);
		}
	});
	...
	$(document).on('click', '#editItem', function (){
        ...
        //reitems[item_id].row.received 		= parseFloat(recieve);
        reitems[item_id].row.qty 			= parseFloat($('#pquantity').val()),
        reitems[item_id].row.real_unit_cost = parseFloat($('#pcost').val()),
		reitems[item_id].row.cost 			= parseFloat($('#pcost').val()),
        reitems[item_id].row.tax_rate 		= new_pr_tax,
        reitems[item_id].tax_rate 			= new_pr_tax_rate,
        reitems[item_id].row.discount 		= $('#pdiscount').val() ? $('#pdiscount').val() : '0',
        reitems[item_id].row.option 		= $('#poption').val(),
        reitems[item_id].supplier_id 		= $('#psupplier').val(),
        reitems[item_id].row.expiry 		= $('#pexpiry').val() ? $('#pexpiry').val() : '';
		reitems[item_id].row.serial 		= ser_arr;
            ...
            <div class="col-lg-12">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <?= lang("repo_reference_no", "repo_reference_no"); ?>
                                <?php echo form_input('reference_no', (isset($_POST['reference_no']) ? $_POST['reference_no'] : $reference), 'class="form-control input-tip" id="reref" style="pointer-events: none;"'); ?>
                            </div>
                        </div>
						<div class="col-md-4">
                            <div class="form-group">
                        ...
                        <div class="col-md-12">
							...
							<div class="row">
								<div class="col-md-12">
									<div class="form-group"> 
										<?= lang("discount_label", "rediscount"); ?>
										<?php echo form_input('rediscount', '', 'class="form-control input-tip" id="rediscount" style="width:30%; pointer-events: none;"'); ?>
									</div>
                            ...
                            <div id="bottom-total" class="well well-sm" style="margin-bottom: 0;">
                                <table class="table table-bordered table-condensed totals" style="margin-bottom:0;">
                                    <tr class="warning">
										...
										<td>
                                            <?= lang('discount') ?>
                                            <span class="discount pull-right" id="tds">0.00</span>
                                        </td>
            ...
            <div class="modal-body" id="pr_popover_content">
                <form class="form-horizontal" role="form">
                    ...
                    <?php if ($Settings->tax1) { ?>
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                                ...
                                echo form_dropdown('ptax', $tr, "", 'id="ptax" class="form-control pos-input-tip" style="width:100%;pointer-events: none;"');
                                ?>
                            </div>
                        </div>
                    <?php } ?>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            <input type="text" class="form-control" id="pquantity" style="pointer-events: none;">
                        </div>
                    </div>
                    ...
                    <?php if ($Settings->product_discount) { ?>
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                                <input type="text" class="form-control" id="pdiscount" style="pointer-events: none;">
                            </div>
                        </div>
                    <?php } ?> 
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            <input type="text" class="form-control" id="pcost" style="pointer-events: none;">
                        </div>
                    </div>
                    ...
                    <input type="hidden" id="qtyreceive" value=""/>

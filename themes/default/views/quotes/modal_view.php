            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover table-striped print-table order-table">
                        ...
                        <tr>
                            ...
                            <td style="text-align:right; width:100px;">
                                <?php
                                    ...
                                    if ($row->method == 1) {
                                        $p_tax = $row->tax;
                                        $ptax = strpos($p_tax, $percentage);
                                        if ($ptax !== false) {
                                            $pds = explode("%", $p_tax);
                                            $pbt = (($row->net_unit_price) / (1 + ($pds[0] / 100)));
                                        }
                                    } else {
                                        $pbt = $row->net_unit_price;
                                    }
							...
							<?php
								if ($Settings->tax1) {
									...
									echo '<td style="width: 100px; text-align:right; vertical-align:middle;">' . 
                                        ($row->item_tax != 0 && $row->tax_code ? '<small>('.$tax.')</small>' : '') . ' ' . 
                                        $this->erp->formatMoney($row->item_tax/$row->quantity) . '</td>';
								}
							?>
							<td style="text-align:right; width:100px;">
                                <?php
                                    if($row->method == 1){
                                        echo $this->erp->formatMoney($row->net_unit_price);
                                    }else{
                                        echo $this->erp->formatMoney($row->net_unit_price+$row->item_tax);
                                    }
                                ?>
                            </td>
                            ...
                            ?>
							<td style="text-align:right; width:120px;"><?= $this->erp->formatMoney($row->final_total); ?></td>
                        </tr>
                        <?php
                        $r++;
                    endforeach;
					...
					<?php
						...
						<tr>
							<td colspan="<?= $col; ?>"
								style="text-align:right; font-weight:bold;"><?= $deposit->note; ?>
								(<?= $default_currency->code; ?>)
							</td>
							<td style="text-align:right; padding-right:10px; font-weight:bold;"><?= $this->erp->formatMoney($deposit->amount); ?></td>
						</tr>
					<?php 
							}
						}
                    ...
                    <tr>
                        ...
                        <td style="text-align:right; padding-right:10px; font-weight:bold;"><?= $this->erp->formatMoney($inv->final_total - $total_deposit); ?></td>
                    </tr>
                    </tfoot>
                </table>
            </div>
                ...
                <div class="buttons">
					...
					?>
						<div class="btn-group" style="float:right;">
                            <a href="<?= site_url('services/modal_view/' . $inv->service_id) ?>" class="tip btn btn-primary" title="<?= lang('services') ?>" data-toggle="modal" data-target="#myModal2"  >
                                <i class="fa fa-server"></i>
                                <span class="hidden-sm hidden-xs"><?= lang('services') ?></span>
                            </a>
                        </div>
					<?php							
						}

...
<div class="row">
    <div class="col-lg-12">
        ...
        <?php $this->erp->qrcode('link', urlencode(site_url('products/view/' . $product->id)), 2); ?>
        <img src="<?= base_url() ?>assets/uploads/qrcode<?= $this->session->userdata('user_id') ?>.png"
             alt="<?= site_url('products/view/' . $product->id); ?>" style="float:right;"/>
        <?php $br = $this->erp->save_barcode($product->code, $product->barcode_symbology, 70, false); ?>
                                    ...
                                    foreach ($options as $option) {
                                        if ($option->wh_qty != 0) {
                                            ...
                                            /*if($Owner || $Admin && (!$Customer || $this->session->userdata('show_cost'))) {
                                                echo '<td class="text-right">'.$this->erp->formatMoneyPurchase($option->cost).'</td><td class="text-right">'.$this->erp->formatMoney($option->price).'</td>';
                                            }*/
                                            echo '</tr>';
                                        }

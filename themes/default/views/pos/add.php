                ...
                <ul class="nav navbar-nav pull-right">
                    <li class="dropdown">
                        <a class="btn account dropdown-toggle" data-toggle="dropdown" href="#">
                            <img alt="" src="<?= $this->session->userdata('avatar') ? site_url() . 'assets/uploads/avatars/thumbs/' . $this->session->userdata('avatar') : $assets . 'images/' . $this->session->userdata('gender') . '.png'; ?>" class="mini_avatar img-rounded"><br>
                ...
                <ul class="nav navbar-nav pull-right">
                    ...
                    <li class="dropdown">
                        <a class="btn bdarkGreen pos-tip" id="register_details" title="<?= lang('register_details') ?>" data-placement="bottom" data-html="true" href="<?= site_url('pos/register_details') ?>" data-toggle="modal" data-target="#myModal">
                            <i class="fa fa-check-circle"></i><p><?= lang('register_details'); ?></p>
                        </a>
                    </li>
                    <li class="dropdown">
                        <a class="btn borange pos-tip" id="close_register" title="<?= lang('close_register') ?>" data-placement="bottom" data-html="true" href="<?= site_url('pos/close_register') ?>" data-toggle="modal" data-target="#myModal">
                            <i class="fa fa-times-circle"></i><p><?= lang('close_register'); ?></p>
                        </a>
                    </li>
                    <li class="dropdown">
                        <a class="btn borange pos-tip" id="add_expense" title="<span><?= lang('add_expense') ?>" data-placement="bottom" data-html="true" href="<?= site_url('purchases/add_expense') ?>" data-toggle="modal" data-target="#myModal">
                            <i class="fa fa-dollar"></i><p><?= lang('add_expense'); ?></p>
                        </a>
                    </li>
					<?php if ($Owner) { ?>
                        <li class="dropdown">
                            <a class="btn bdarkGreen pos-tip" id="today_profit" title="<?= lang('today_profit') ?>" data-placement="bottom" data-html="true" href="<?= site_url('reports/profit') ?>" data-toggle="modal" data-target="#myModal">
                                <i class="fa fa-hourglass-half"></i><p><?= lang('Today_Profit'); ?></p>
                            </a>
                        </li>
                    <?php } ?>
                    <?php if ($Owner || $Admin) { ?>
								...
								<div class="row">
									...
									<?php } ?>
										<?php 
											...
											echo form_dropdown('sale_status', $sst, (isset($_POST['sale_status']) ? $_POST['sale_status'] : 'completed'), 'class="form-control input-tip" required="required" id="sale_status"'); 
										?>
									</div>
								</div>
                            ...
                            <div id="left-bottom">
                                <table id="totalTable"
									...
									<tr>
                                        ...
                                        <td class="text-right">
                                            <?//= lang('rate'); ?> <span style="padding: 5px 10px;font-size: 16px; font-weight:bold;display:none;" id="khmer_rate" ><?php echo $exchange_rate->rate?number_format($exchange_rate->rate):0 ?>  ៛</span>
                                        </td>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right" style="padding:5px 10px 5px 10px; font-size: 30px; border-top: 1px solid #666; font-weight:bold; background:#333; color:#FFF;" colspan="2">
                                            <span style="float:left;display:none;" id="gtotal_kh"></span>
                                ...
                                <div id="botbuttons" style="text-align:center;">
                                    ...
                                    <input type="hidden" name="sale_status_1" id="sale_status_1" value=""/>
                                ...
                                <div id="payment-con">
                                    <?php for ($i = 1; $i <= 5; $i++) { ?>
										...
										<input type="hidden" name="other_paid[]" id="other_paid_<?= $i ?>" value=""/>
										...
										<tbody>
											<tr>
												...
												<?php
													...
													{
												?>
														<th  style="text-align:center;"><?=$row->country?></th>
												<?php
													}
												?>
											</tr>
											...
											<tr>
												<td width="50%" style="height: 50px;">
                                                    <?= lang("paid_amount"); ?>
                                                </td>
												<td class="text-right">
                                                    <input name="amount[]" type="text" id="amount_1" class="pa form-control input-lg kb-pad amount" style="text-align:right;"/>
                                                </td>
												<?php
													foreach ($query as $row)
													{
												?>
														<td class="text-right">
															<input name="other_cur_paid[]" rate="<?=$row->rate?>" type="text" id="other_cur_paid" class="form-control input-lg kb-pad currencies_payment kh_paid_1" style="text-align:right;"/>
														</td>
												<?php
													}
												?>
											...
											<tr>
												...
												<td class="text-right">
                                                    <span id="remain" class="main_remain">0.00</span>
                                                </td>
												<?php
													foreach ($query as $row)
													{
											...
											<tr>
												...
												<td class="text-right"><span id="change" class="main_change">0.00</span></td>
												<?php
												$this->db->select('*');
												$this->db->from('erp_currencies');
                                        ...
                                        <div class="col-sm-11">
                                                    ...
                                                    <div class="col-md-3">
                                                        <div class="form-group">
															<select name="cc_type[]" id="pcc_type_1"
																...
																<option value="diners_club"><?= lang("Diners Club"); ?></option>
																<option value="jcb"><?= lang("JCB"); ?></option>
																<option value="union_pay"><?= lang("Union Pay"); ?></option>
                                            ...
                                            <div class="pbank_1" style="display:none;">
                                                <div class="form-group">
                                                    ...
                                                    <input name="bank_transfer[]" type="text" id="bank_transfer_1" class="form-control bank_transfer"/>
                                                </div>
                                            </div>
<!--
                                            <div class="form-group">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
				...
				<h4 class="modal-title" id="prModalLabel1"></h4>
            </div>
            <div class="modal-body" id="pr_popover_content">
                <form class="form-horizontal" role="form">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            <input type="text" class="form-control kb-pad" id="pquantity" readonly>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
							...
							<?php } else { ?>
								<input type="text" readonly class="form-control kb-pad" id="pprice">
							<?php } ?>
                        </div>
                    </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body scroll_F" id="pr_popover_content" style="height:400px;overflow:hidden;">
                <form class="form-horizontal" role="form" id="s_seModal">
					<div class="col-lg-5"></div><div class="col-lg-1" style="float: left; text-transform: uppercase; top: 5px;">search</div><div class="col-lg-6" style="margin-bottom: 5px; padding-right: 0px;"><input id="search_all" class="form-control" type="text"></div>
					<table style="width: 100%;">
						<thead>
							<tr>
								<td style="padding:0;margin:0;border:1px;"><input type="text" style="width:188px; border-right:none" class="form-control" id="Pcode"/></td>
								<td style="padding:0;margin:0;border:1px;"><input type="text" style="width:145px;border-right:none" class="form-control" id="Pname"/></td>
								...
								<td style="padding:0;margin:0;border:1px;"><input type="text" style="width:102px;border-right:none" class="form-control" id="Pcategory" ></td>
								<td style="padding:0;margin:0;border:1px;"><input type="text" style="width:68px;border-right:none" class="form-control" id="Pprice"></td>
								<td style="padding:0;margin:0;border:1px;"><input type="text" style="width:252px;" class="form-control" id="Pstrap"></td>
							</tr>
						</thead>
					</table>
                    <table class="table table-bordered">
						<thead>
							<tr>
								...
								<th style="width:128px"><?= lang("product_code");?></th>
								<th style="width:134px"><?= lang("product_name");?></th>
								<th style="width:104px"><?= lang("description");?></th>
								<th style="width:94px"><?= lang("category")?></th>
								<th style="width:63px"><?= lang("price");?></th>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
		...
		if (localStorage.getItem('sale_status')) {
		...
		$(document).on('change keyup paste', '#other_cur_paid', function () {
		...
		$('#paymentModal').on('change', '.kh_paid_<?=$i?>', function (e) {
            $('#other_paid_<?=$i?>').val($(this).val());
        });
        $('#paymentModal').on('blur', '.kh_paid_<?=$i?>', function (e) {
        ...
        $('#payment').click(function () {
			...
			if(Owner || Admin || (GP == 1)){
				...
				var order_dis = $('#posdiscount').val();
				
				if(val == 0 && order_dis == 0){
					<?php if ($sid) { ?>
					suspend = $('<span></span>');
					suspend.html('<input type="hidden" name="delete_id" value="<?php echo $sid; ?>" />');
					suspend.appendTo("#hidesuspend");
			...
			if (sale_status == 'ordered') {
				...
				$('#other_cur_paid').attr('disabled', true);
				$('.addButton').attr('disabled', true);
				$('.quick-cash').attr('disabled', true);
			}
		});
        ...
        $(document).on('click', '.addButton', function () {
            if (pa <= 5) {
				...
				$('#paid_by_1, #pcc_type_1, #paid_by_' + pa + ', #pcc_type_' + pa).trigger('change');
                read_card();
                pa++;
            } else {
        ...
        $(document).on('click', '.product', function (e) {
            ...
            $.ajax({
                type: "get",
                url: "<?= site_url('pos/getProductDataByCode') ?>",
                data: {aaa:"AAA",code: code, warehouse_id: wh, customer_id: cu, suspend_id: susp_id, item_rows: item_row, sub_total: subtotal},
                dataType: "json",
                success: function (data) {
		...
		*/
		...
		$(document).on('keyup', '#amount_1, #amount_2, #amount_3, #amount_4, #amount_5, .currencies_payment', function(){
			...
			var paid 	 = us_paid + us_paid2 + us_paid3 + us_paid4 + us_paid5 + other_paid;
		...
		$("#search_all").autocomplete({
            ...
            source: function (request, response) {
                if (!$('#poscustomer').val()) {
                    $('#search_all').val('').removeClass('ui-autocomplete-loading');
                    bootbox.alert('<?=lang('select_above');?>');
                    //response('');
                    $('#search_all').focus();
                    return false;
			...
			var inner_html  = 	'<td style="width:50px;height:30px;">' +
								...
								'<td style="width:64px;">'+
									item.strap +
								'</td>' +
								'<td class=""><a href="#ShowImage" data-toggle="modal"><img class="img-circle passimage" src="' + '<?= base_url() ?>' + 'assets/uploads/thumbs/'+item.pic+'" alt="'+item.pic+'" style="width:30px; height:30px;"></a></td>';
			return $( "<tr></tr>")
		...
		$('#search_all').bind('keypress', function (e) {
            if (e.keyCode == 13) {
                e.preventDefault();
		...
		$(document).on("click", ".passimage", function () {
			...
			var image = '<img src="' + '<?= base_url() ?>' + 'assets/uploads/'+img+'" style="width:100%;"/>';
			$('.getImg').html(image);
		});
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data === false) {
                            ...
                            $('#dp_details').html('<small>Customer Name: ' + data.name + '<br>Amount: <span class="deposit_total_amount">' + (data.deposit_amount == null ? 0 : data.deposit_amount) + '</span> - Balance: <span class="deposit_total_balance">' +(data.deposit_amount - amount) + '</span></small>');

					...
					<tbody>
						<?php
							...
							foreach ($rows as $row) {
								...
								echo '<tr ' . ($row->product_type === 'combo' ? '' : 'class="items"') . '>
										<td class="text-left">#' . $r . "</td>";
								echo '	<td class="text-left">' . 
											product_name($row->product_name) . 
											' ('.$row->product_code .') '.
										($row->variant ? ' (' . $row->variant . ')' : '') . '</td>';
					...
					<tfoot>
						...
						<?php
							...
							if ($inv->order_discount != 0) {
								echo '<tr><th colspan="' . $colspan . '" class="text-right" style="border-top:2px dotted #ddd;padding-right: 12px;">' . lang("order_discount") . '(' . $inv->order_discount_id . ')</th><th style="border-top:2px dotted #ddd #ddd" class="text-right">'.$this->erp->formatMoney($inv->order_discount) . '</th></tr>';
							}
							$amount_kh_to_us = 0;
										...
										?>
											<table style="width:100%;">
												...
												<tr>
													...
													<th style="border:2px solid #000;border-left:none;" class="text-right"><?= $this->erp->formatMoney($payment->pos_paid_other) . '  ៛' ; ?></th>
												</tr>
												<?php
													}
												?>
						...
						<?php
								...
								if(count($payments) > 1){
									...
									$exrate = 0;
									if($pay_kh != 0 || $exchange_rate->rate != 0){
										$exrate = ($pos_settings->in_out_rate) ? $outexchange_rate->rate : $exchange_rate->rate;
									}
									if(($pay + $exrate - $inv->grand_total) != 0){
						...
						<?php
							...
							if(count($payments) > 1){
								...
								$exrate = 0;
								if($pay_kh != 0 && $exchange_rate->rate != NULL){
									$exrate = $pay_kh / (($pos_settings->in_out_rate) ? $outexchange_rate->rate:$exchange_rate->rate);
								}
								if((($pay + $exrate) - $inv->grand_total) != 0){
					?>
									<tr>

...
class Auth_model extends CI_Model
{
    ...
    public function check_password_db($password, $use_sha1_override = FALSE)
    {
        if (empty($password)) {
            return FALSE;
        }
        ...
        $hash_password_db = $query->result();

        // bcrypt
        if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt') {

            foreach ($hash_password_db as $password_db) {
        ...
        if ($this->store_salt) {
            foreach ($hash_password_db as $password_db) {
                $db_password[] = sha1($password . $password_db->salt);
            }
        } else {
            foreach ($hash_password_db as $password_db) {
                $salt = substr($password_db->password, 0, $this->salt_length);
        ...
        $i = 0;
        foreach ($hash_password_db as $password_db) {
            if ($db_password[$i] == $password_db->password) {
                return TRUE;
            } else {
                return FALSE;
            }

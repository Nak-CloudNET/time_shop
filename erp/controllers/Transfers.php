...
class Transfers extends MY_Controller
{
    ...
    function getTransfers()
    {
		...
		$transfer_back = anchor('transfers/transfer_back/$1', '<i class="fa fa-refresh"></i> ' . lang('transfer_back'), 'class="transfer-back"');
        ...
        $delete_link = "<a href='#' class='tip po' title='<b>" . lang("delete_transfer") . "</b>' data-content=\"<p>"
            . lang('r_u_sure') . "</p><a class='btn btn-danger' id='a__$1' href='" . site_url('transfers/delete/$1') . "'>"
	...
	function getInTransfers()
    {
        ...
        $this->datatables
            ->select("	transfers.id as idd, 
						...
						SUM(".$this->db->dbprefix('transfer_items').".quantity) as qty ,transfers.status")
            ->from('transfers') 
			->JOIN('transfer_items','erp_transfer_items.transfer_id=erp_transfers.id','LEFT')
    ...
    function add()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $shipping = 0;//$this->input->post('shipping');
    ...
    {
        $this->erp->checkPermissions('delete', TRUE, 'transfers');
    ...
    function suggestions()
    {
        ...
        if ($rows) {
			foreach ($rows as $row) {
				...
				$row->sep 			 	= $row->serial_no;
                $combo_items 		 	= FALSE;
				
                if ($row->tax_rate) {
                    $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
                    if ($row->type == 'combo') {
                        $combo_items = $this->transfers_model->getProductComboItems($row->id, $warehouse_id);
						$row->sep = $this->transfers_model->getComboSerial($row->id, $warehouse_id);
                    }
	...
	function getProductSerialAjax($serial = NULL){        
        $row = $this->transfers_model->getSerialProducts($serial);
        if ($row) {
            echo 1;
        } else {
            echo 0;
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('E1', lang('total_qty'));
                    ...
                    foreach ($_POST['val'] as $id){
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $tansfer->total_quantity);
    ...
    {
        $this->erp->checkPermissions('add', true, 'transfers');
        ...
        if ($this->form_validation->run()) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_expiry 	= isset($_POST['expiry'][$r]) ? $this->erp->fsd($_POST['expiry'][$r]) : NULL;
                $item_option 	= isset($_POST['product_option'][$r]) && $_POST['product_option'][$r] != 'false' && $_POST['product_option'][$r] != 'null' ? $_POST['product_option'][$r] : NULL;
				$serial_number  = $_POST['serial'][$r];

                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                    $product_details = $this->transfers_model->getProductByCode($item_code);
					$warehouse_quantity = $this->transfers_model->getWarehouseProduct($from_warehouse_details->id, $product_details->id, $item_option);
					$combo_quantity     = $this->transfers_model->getWarehouseProductCombo($from_warehouse_details->id, $product_details->id);
					if($product_details->type == 'combo'){
						foreach($combo_quantity as $combo_qty){
							if ($combo_qty->quantity < $item_quantity) {
								$this->session->set_flashdata('error', lang("no_match_found") . " (" . lang('product_name') . " <strong>" . $combo_qty->name . "</strong> " . lang('product_code') . " <strong>" . $combo_qty->code . "</strong>)");
								redirect("transfers/add_in_transfer");
							}
						}
					}else{
						if ($warehouse_quantity->quantity < $item_quantity) {
    ...
    {
        $this->erp->checkPermissions('edit', true, 'transfers');
        ...
        if ($this->form_validation->run() == true && $this->transfers_model->updateTransfer($id, $data, $products)) {
            ...
            foreach ($transfer_items as $item) {
                $row 					= $this->site->getProductByID($item->product_id);
				$serial					= $this->transfers_model->getProductAllSer($item->product_id, $item->warehouse_id);
                if (!$row) {
                    $row = json_decode('{}');
                } else {
                    unset($row->cost, $row->details, $row->product_details, $row->supplier1price, $row->supplier2price, $row->supplier3price, $row->supplier4price, $row->supplier5price);
				...
				$combo_items = FALSE;
                if ($row->type == 'combo') {
                    $combo_items = $this->transfers_model->getProductComboItems($row->id, $item->warehouse_id);
					$row->sep = $this->transfers_model->getComboSerial($row->id, $item->warehouse_id);
                    $te = $combo_items;
	...
	function transfer_back($id = NULL){
		...
		if($this->transfers_model->transferBack($data, $products)){
			$this->session->set_userdata('remove_tols', 1);
